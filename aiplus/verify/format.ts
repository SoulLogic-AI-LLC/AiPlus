...
export function formatVerifyHistory(entries: VerifyEntry[]): string {
  ...
  lines.push(
    `  ${C.dim}ID${C.reset}                    ${C.dim}Verdict${C.reset}  ${C.dim}Session${C.reset}                  ${C.dim}Time${C.reset}`,
  )
  ...
  for (const entry of entries) {
    ...
    lines.push(
      `  ${truncate(entry.id, 22).padEnd(22)} ${vc}${vi} ${entry.verdict.padEnd(6)}${C.reset} ${sid} ${formatTimeAgo(entry.timestamp)}`,
    )
  }
