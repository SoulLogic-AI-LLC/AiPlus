...
describe("DaemonLifecycle", () => {
  test(
    "active turn holds off shutdown, idle does not auto-shutdown",
    () =>
      Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
  ...
  test(
    "shutdown waits for active turn to finish within grace period",
    () =>
      Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
  ...
  test(
    "shutdown does not wait longer than shutdownGraceMs",
    () =>
      Effect.runPromise(
        Effect.scoped(
          Effect.gen(function* () {
